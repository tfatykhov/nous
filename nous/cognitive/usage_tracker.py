...
class UsageTracker:
    ...
    def record_retrieval(
        ...
        if memory_id not in self._stats:
            self._stats[memory_id] = MemoryUsageStats(memory_id=memory_id, memory_type=memory_type)
    ...
    def _prune_old_records(self) -> None:
        ...
        for memory_id, records in self._records.items():
            ...
            self._records[memory_id] = [r for r in records if (now - r.retrieved_at).total_seconds() < cutoff_seconds]

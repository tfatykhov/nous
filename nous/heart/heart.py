...
class Heart:
    ...
    async def list_facts_by_category(
        ...
        return await self.facts.list_by_category(
            ...
            learned_within_days=learned_within_days,
        )
    ...
    async def set_fact_tag(self, fact_id: UUID, tag: str, present: bool, session: AsyncSession | None = None) -> None:
    ...
    async def _recall(
        ...
        if self.settings.query_expansion_enabled and self._query_expander is not None and self._embeddings is not None:
            try:
            ...
            try:
                ...
                if owns_session:
                    try:
                        ...
                        logger.warning(
                            "Recall sub-search rollback failed for %s; remaining sub-searches will likely fail",
                            memory_type,
                            exc_info=True,
                        )
        ...
        if residual_activations and self._residual_activator is not None:
            try:
        ...
        if apply_mmr is True:
            ...
            if mmr_blocked_by_ce:
                logger.info("MMR: skipped (F030.1 — CE reordered head, mmr_skip_after_ce=True)")
        ...
        if mmr_active and len(merged) > 1 and self._embeddings is not None:
            try:

...
class Settings(BaseSettings):
    ...
    spreading_score_depth1_parity: bool = Field(
        ...
        description=(
            ...
            "PRECONDITION (codex P2): parity is defined relative to a SCORING "
            "POLICY, so this is gated on the policy in force. `seed * w` is what "
            "the 1-hop leg returns only on its seed-score branch with no "
            "provenance penalty — i.e. `graph_neighbor_seed_score_enabled=true` "
            "AND `graph_inferred_edge_penalty >= 1.0`, which is prod. At seed "
            "0.8 / w 0.9 the 1-hop leg returns 0.7200 there, 0.6300 with "
            "seed-score off, and 0.5040 with an F065 penalty of 0.7, while the "
            "parity branch always returns 0.7200 — so ungated this flag would "
            "mean parity, a mild promotion or a large one depending on two "
            "unrelated settings, and the C-S arm could not attribute its own "
            "result. Where the policy does not match, the flag goes INERT "
            "(today's scoring) rather than silently promoting. The penalty "
            "cannot be mirrored onto a spreading row instead: an activation "
            "composes several edges of mixed provenance, so there is no single "
            "`extraction_method` to price. "

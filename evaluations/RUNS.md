...
| 2026-05-05 — | other | feat/F060 | f060-validation-* (synthetic) | 5 mixed fixture (3 full + 1 fallback + 1 abandon) | 5 | F060+F060.1+F060.2 | **3/3 full transcript recovered, 1/1 summary-fallback recovered, 1/1 marked active=false outcome=abandoned** | scripts/eval/validate_f060.py (OVERALL PASS = True) | **All 3 paths validated** — episodes from A/B stay active=true with structured_summary; path C row goes active=false |
| 2026-05-05 — | other | feat/F060 | nous-default (live prod data) | active+stuck-open partition | 103 | F060 path classification | 0 path-A (no transcripts), 93 path-B (summary fallback), 10 path-C (mark abandoned), 0 too-new | direct prod psql probe | **prod first-cycle prediction** — 60 cleared cycle 1 (50 path-B cap + 10 path-C), all 103 resolved within 2 cycles |
| 2026-05-05 — | compaction | feat/F060 | nous-prod-snapshot | 10 real stuck-open episodes (transcript NULL, summary >= 20 chars) | 10 | F060.1 fallback quality probe — `summarize_episode(transcript=plain_summary)` + F059 entity guard | **8 summarized, 0/8 above F059 threshold; 5/8 fired regex FPs (sentence-start capital phrases); avg 1.9 facts emitted** | reports/eval_f060_1_fallback_quality.md | **F060.1 safe to enable** — no fabricated entities (emails/names/numbers); summaries faithful to user intent; titles slightly overclaim (summarizer-prompt issue, not F060.1) |

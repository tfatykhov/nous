...
| `NOUS_SUPERSESSION_POLICY` | `ordinal` | F084 R2.2 winner rule for UPDATE conflicts: `ordinal` (higher `source_ordinal` wins, same-episode only; falls back to `recency`) or `recency` (later `learned_at` wins). CONTRADICTION resolution always uses the classifier's `current_fact` verdict — ordinal/recency never apply. `authority` reserved. |

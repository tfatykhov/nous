...
| `NOUS_CLAIM_VERIFICATION_MODE` | `enforce` | Claim verification mode (shadow/warn/enforce). Harness Phase 2c. **What is a claim:** only the agent's own completed action — "I saved…", "I've pushed the fix", "I saved X and sent Y", plus actor-less completions with a completion shape ("was saved to <path>", a line-initial "Email sent to …"). Not a claim: narrating another agent's work ("the node ran and sent the email"; the `and <verb>` rule is clause-scoped), a question, "if/whether/unless I…", quoted text (fenced code, `>` quote lines, "…" spans), a lowercase "i", push/commit without a version-control object ("I pushed an approval card" is `push_surface` wording), or an effect that landed in memory, the companion app, the dashboard or the reply itself ("below"). A claim never spans a sentence, and its target is only where the object was saved. **What grounds it:** any tool capable of the effect that succeeded with arguments showing it — `write_file`, a bash write, or a script that writes a file; `send_email`, `send_file`, a mail client/`msmtp`/`swaks`, SMTP or Telegram `curl`, or a script using `smtplib`; `git push` (not `-n`/`--dry-run`) or `docker push`; `git commit` (`-n` is `--no-verify`); a deploy CLI, a `deploy` script or `git push` for a deploy — and a named path or recipient must match. Bash is read with the ledger classifier's own lexer (`bash_side_effect.command_invocations`), so `git push … \| tail -n 5` and `if git push; then …` push while `echo "git push"` does not. **Unreadable is not no-signal:** a command the reader cannot parse (a heredoc body with an apostrophe) or one running something opaque (`bash -c`, `eval`, `ssh`, an interpreter, a script, `make`/`npm`) cannot disprove a claim and counts as plausible — under `enforce` a false violation misleads the agent, so only a command that was READ and did something else is disproof. A non-zero exit disproves only an effect of the last command. The ledger reads a command's invocations at record time from the whole command; this turn's arguments are checked untruncated; everything is linear, since it runs on the event loop. `enforce` injects a correction into the next turn (streaming cannot unsend). The `f026_claim_verification` event records every claim with its evidence level (`exact`/`plausible`/`none`), `claim_count` and the `turn`, so precision is measurable. |

...
| `NOUS_RETRIEVAL_TELEMETRY_SNIPPET_CHARS` | `200` | F091 per-candidate content truncation. Full fact bodies are never stored — that is both unbounded growth and a needless copy of user content into a diagnostic table. |
...
| `NOUS_RETRIEVAL_TELEMETRY_RING_SIZE` | `100` | F091 in-memory ring of recent traces for live dashboard reads before the DB write lands. |

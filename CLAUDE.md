...
| `NOUS_SPREADING_HEART_SEEDS_ENABLED` | `false` | F022 extension (2026-07-11, **land-dark**): seed the spreading CTE with the top-3 heart FACT results (RRF-scored) alongside decision seeds — spreading fires on decision-less corpora and leverages the fact/chunk graph instead of decisions only. Flip only after the retrieval A/B. |

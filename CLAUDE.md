...
| `NOUS_RETRIEVAL_TELEMETRY_MAX_CANDIDATES` | `600` | F091 hard per-row candidate cap; hitting it sets `truncated` and logs WARNING (never silent). **Raised 300 → 600 by C1**, which registers the chunk leg's discard set — the largest population on the path. At the live config (`NOUS_EPISODE_CHUNK_RECALL_LIMIT=30` with **no** `NOUS_CHUNK_RRF_PENALTY_LIMIT` set in either `.env`), `limit_expanded = 90` per leg, so up to `2*90 − 30 = 150` discards land on top of the ~110–150 already registered. At 300 the cap would have moved from anomaly signal to steady state — `truncated` on every sampled row, the dashboard cap banner permanently lit, and, worst, a gold chunk cut at the merge but past the cap recorded nowhere, so it still reads `never_retrieved`: the exact miscount C1 exists to fix. `Leg.n_dropped` is exact regardless of this value, so truncation costs identity, never the count. |

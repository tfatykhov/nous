...
| `NOUS_TOOL_CONTEXT_POLICY_MODE` | `warn` | Harness Phase 2a: per-context tool policy (`nous/api/tool_policy.py`), consulted at the same choke point as the offered-set check (`AgentRunner._authorize_tool_call`, both loops). Every tool's class (`none`/`write`/`external`/`irreversible` + `spawns`) is declared once in `nous/api/tool_classes.py`; the ledger's `READ_TOOLS`/`WRITE_TOOLS`/`EXTERNAL_TOOLS`/`IRREVERSIBLE_TOOLS` and the F078 refuse denylist derive from it (the denylist now also strips the 8 tools that had no class — `ingest_document`, `resolve_decision(s)`, `spawn_sync`, `dag_create`, `dag_manage`, `push_surface`, `compose_surface`), a drift test fails on any `dispatcher.register()` without an entry, and `run_python` code or a `python -c` string that reaches the network (`smtplib`, `requests`, `httpx`, `urllib`, `socket`, a URL literal — erring toward external by design) classes as `external` instead of a local write. Rows: `interactive`/`mcp` may do anything; `subtask`/`dag_node`/`scheduled`/`agent_action` may send but not spawn; `dag_summary` may spawn only `spawn_task` (prod delivers DAG mail through it); `heartbeat_triage` and generic `background` are local-only (`none`/`write` — by any route, `bash curl` and a networking script included); a `heartbeat_check`/`heartbeat_callback` with a declared tool list (`ExecutionContext.declared_tools`; an empty list is the DB default and means undeclared) may use only those tools, declaring `heartbeat_check_create` authorizes a check pipeline, and a callback may not `heartbeat_check_manage(action=enable)` its own check (`ExecutionContext.check_name`). Violation codes: `unclassified`, `undeclared`, `level:<level>`, `spawn`, `reenable`. `off` = no check; `warn` = run it, log WARNING, persist `harness_context_policy_violation` (`tool_name`, `context_kind`, `violation`, `mode`); `enforce` = refuse with a paired tool error and a `blocked` ledger row (code `context_policy`). Both rules run for every call, so a warn-mode deviation is recorded by both. Ships `warn`: Phase 1a/1b/2c are not deployed yet, so no prod data exists — flip after reading the events. |

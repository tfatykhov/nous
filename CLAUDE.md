...
| `NOUS_EMBEDDING_CACHE_SIZE` | `1024` | Audit D2/S7 (2026-06-09): bounded in-process LRU on EmbeddingProvider, keyed (model, dimensions, sha256(text)). Eliminates the 4–7× repeat query embeds per recall and repeat content/template embeds per learn + sleep cycle. `0` disables. ~13 MB ceiling at 1024 entries. |

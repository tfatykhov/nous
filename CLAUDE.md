...
| F082 | PPR Recall Leg (query-personalized PageRank over `brain.graph_edges` — HippoRAG-v2 primitive; N-leg weighted RRF refactor, power-iteration `nous/brain/ppr_recall.py`, density gate, autobehavior exclusions reused; default OFF `ppr_weight=0.0`; `ppr_recall_enabled=auto` density-gated; 38 new tests; AC1 2-leg bit-identical back-compat verified) | — |
...
| `NOUS_PPR_RECALL_ENABLED` | `auto` | F082: PPR recall leg mode (auto/true/false). `auto` uses density gate. |
| `NOUS_PPR_WEIGHT` | `0.0` | F082: PPR RRF leg weight. **0.0 = completely inert** (no DB query). Set >0 to activate fusion. |
| `NOUS_PPR_DAMPING` | `0.5` | F082: PageRank damping factor d (HippoRAG parity). |
| `NOUS_PPR_TOLERANCE` | `1e-4` | F082: L1 convergence threshold for power iteration. |
| `NOUS_PPR_MAX_ITER` | `30` | F082: Hard iteration cap for PPR power iteration. |
| `NOUS_PPR_LEG_LIMIT` | `20` | F082: Max nodes returned by the PPR leg. |
| `NOUS_PPR_MIN_DENSITY` | `0.0` | F082: Min graph density for auto mode. `<=0` inherits `spreading_activation_density_threshold`. |
| `NOUS_PPR_SEED_TOP_K` | `50` | F082: Max Stage-1 hits to use as PPR reset vector seeds. |

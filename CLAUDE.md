...
| `NOUS_PROGRAMMATIC_TOOLS_TIMEOUT` | `90` | Timeout in seconds for run_python code execution. Enforced in-thread by a `sys.settrace` deadline hook, so Python-level runaway code (`while True: pass`) is actually interrupted; a blocking C-level call still cannot be interrupted and holds its slot until it returns. **Raised 10 → 90 (2026-08-25)** when in-script `recall_deep` stopped being `heart.search_facts` and became the real `run_recall_pipeline`. Measured on prod `nous_system.retrieval_log` (n=104): a pipeline retrieval runs **p50 5.3s / p95 14.7s / max 54.7s**, so the old 10s budget could not fit even ONE call reliably — and batching several is the tool's stated purpose. 90s fits ~6 calls at p95 and stays under `NOUS_TOOL_TIMEOUT` (120s), which wraps every tool in `_dispatch_with_keepalive`; if the inner deadline ever met the outer one, the outer cancel would fire first and swallow both the per-script error message and the F091 partial-trace commit (`test_timeout_leaves_headroom_under_tool_timeout` locks the ordering). **Operator note:** a run now holds one of `NOUS_PROGRAMMATIC_TOOLS_MAX_CONCURRENT` (4) slots for up to 90s rather than 10s — a 9× longer worst-case hold on a fixed-size pool. Lower this if scripts start queueing. |

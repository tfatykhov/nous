...
| F092 | [A2UI Companion App](docs/features/F092-a2ui-companion.md) (Phase 0+1 spine — the agent renders structured interactive UI instead of prose: A2UI v1.0 vendored at pinned commit `d9086fb` under `nous/a2ui/catalogs/`; `SurfaceService` persists authoritative surface state + an outbox delta log (migration 071: `nous_system.a2ui_surfaces/a2ui_outbox/a2ui_actions`), pushes over resumable SSE `GET /a2ui/stream` (SSE id = outbox seq, `Last-Event-ID` WINS over `?since=`, hydration-first reconnect so a surface resolved while offline never zombies); Svelte 5 renderer at `/companion` (second Rollup entry in dashboard-app, adjacency-list walker with cycle/depth guards, RFC 6901 two-way binding, basic-catalog subset + `nous-core` catalog served at `/a2ui/catalog/{name}`); user actions POST `/a2ui/action` through allowlist → nonce → Content-Type (the CSRF control) → rate limit → censor gate → durable audit; `push_surface` agent tool (template-first: approval_gate / action_review / heartbeat_findings, `dedup_key` update-in-place); validator rewrites the vendored `\p{...}` Extensions pattern at load (Python `re` crashes on it) and validates mixed-catalog surfaces against a merged catalog. Deferred: one-store conversation + Telegram echo, `compose_surface` LLM path, graph surfaces, renderer/agent function RPC, PWA) | — |
| F093+F094 | [Micro-App Design System](docs/features/F093-microapp-design-system.md) + [Visualization Vocabulary](docs/features/F094-visualization-vocabulary.md) (the general "build any dashboard" capability — Nous composes a live, self-refreshing micro-app for whatever domain the user needs, instead of emailing static HTML. **F093:** 5 closed themes stamped `data-theme` on the app ROOT, not `:root`, so two open apps theme independently and the global switcher is untouched; the theme rides `createSurface` `metadata.extensions.com_nous_theme` and is persisted in `app_spec`, which is what makes it survive dedup replacement for free (app_spec is already copied) — and it is **pinned on refine**, because `updateComponents` carries no metadata slot at all, so a recomposed theme could only surface on the next reconnect as an ambush. Semantic tokens (`--ok/--warn/--crit/--locked/--soft/--font-display`) whose `:root` defaults equal the aliases they replace, so nous-default renders byte-identical; 17 catalog views migrated off raw colour vars; the light `paper` theme declares its own `color-scheme` since the global one is dark. `Repeat` is `{componentId, path}` template children — **grammar + prompt sugar over the pre-existing `Children.svelte` machinery, zero renderer change**; `_children_of` learning to yield a template's `componentId` is what makes it visible to BOTH the depth walk and the one-parent rule, so double-render rejection comes free. `Section.layout` enum + archetype-aware caps (ledger/briefing 80/8 vs 40/5). **F094:** Sparkline/LineChart/BarChart as hand-rolled SVG (the companion entry ships no vendor JS — d3/cytoscape are dashboard-only), all geometry in a DOM-free `chart.ts` because jsdom implements no SVG layout and any `getBBox()` design would be untestable. Series contract `{kind:series, points:[{t,v}], unit, meta}` + `to_series(records, t_key, v_key)` — **the actual generalizer**: it makes any existing record-list fetcher chartable without a rewrite. Zero-basing is normative for Bar (a truncated bar axis is how a chart lies) and wins before the flat-line pad; Line/Spark draw a `~` break marker; a lone-coordinate segment renders as a dot, since a one-point polyline strokes nothing. Fidelity is the through-line: a non-finite reading is kept as a `t`-only **gap placeholder** so the line breaks rather than bridging, and the 200-point downsampler preserves — per key — the endpoints, the extrema, and one representative gap per broken pair. `_bound_series` downsamples instead of being replaced by a truncation marker, and returns an **honest empty series** when the char budget is exhausted rather than a 2-point stub stamped `downsampled_from:200` (a fabricated trend is the §1.1 "published a false statement" failure; `_TOTAL_BUDGET_CHARS` raised 12k→16k so a 3-chart app renders real data). Data-aware compose validation closes the "partial render of a complete source" class: unread-source, series-shape, series-arity against DECLARED `keys` (never `points[0]`, which false-rejects a valid sparse or empty series), single-value guard so a multi-series source can't silently render "no data" on a Sparkline, and coverage counted as distinct IN-RANGE bound indices. `nous_system.dag_nodes` has no `agent_id`, so `dag_throughput_series` JOINs `execution_dags` for scoping. Charts are enum-only by **schema proof**: `style`/`color` props are rejected by `unevaluatedProperties: false`, stronger than a grep. 9 codex rounds + 2 peer reviews; 26 findings fixed, each with a regression test) | #616 |
| F096 | [Report Vocabulary](docs/features/F096-report-vocabulary.md) (a micro-app could draw a trend after F094 but could not say what it MEANS — no verdict card, no tone-coloured delta, no ranked movers, no real table, no status chip — so every "how are things going" report still left the companion as emailed HTML. Generalized from a hand-authored health trend report whose content counts down to **five element classes**: `MetricCard` (label · tone pill · preformatted value+unit · caption · embedded trend · footnote — the VALUE is never coloured, tone lands on the judgement), `ScoreCard` (verdict pill + evidence rows with per-row tone, top rule not a left stripe), `DeltaList` (`label · delta · from → to`, an empty list is a STATE with `emptyText`), `DataTable` (≤6 literal columns, `align: end` tabular figures, `secondary`), `ChipRow`; plus `Section.caption` (attribution, a DynamicString), `Section.layout: cards` (≥220px auto-fit — `grid-3`'s 140px smears a spark), `AppHeader.note` ("data through …"), and Sparkline treatment: an end dot (skipped when the last reading is already an isolated dot), an opt-in `trendline` whose rolling mean is computed PER FINITE RUN so it never bridges a gap, and a **source-declared** focus window (`to_series(..., focus_from=)` → `meta.focus_from`) — series META, not a prop, because the window boundary belongs to whoever computed the comparison; **no area fill**, since F094 makes zero-basing normative for filled charts and a sparkline is not zero-based. `MetricCard`/`ScoreCard.tone` admits a bare `{path}` binding (literal stays enum-closed) because a metric grid is ONE template under a Repeat and a literal tone would paint every card the same. The one backend fix the reference shape forces: a metric grid is a record list with a series INSIDE each record, and `_bound` popped records (17 metrics × 56-point sparks ≈ 34k vs the 12k per-source budget kept four); it now shortens every VALID embedded series first — gated on `_is_valid_series`, or a malformed one would 500 `resolve()`. Compose validation gains ONE shared `_resolve_path_targets` (charts, `MetricCard.trend`, and a new array rule for six list components) that walks `grammar._children_of` — the chart-only walker would have false-rejected a KeyValueTable inside `Modal.content` inside a Repeat; optional data props resolving to nothing are a state, not an error. `report` theme (cool slate, system serif display, `--soft` deliberately lighter than `--muted`) + `report` archetype at 80/10 caps; `_build_prompt` caps its data sample PER SOURCE (a report declares 5–7 and the tail was cut mid-JSON) and tags a records source whose records carry a series. Spec reviewed by a 3-lens adversarial workflow (79 agents, 2 refuters per finding): 29 confirmed findings folded, 9 refuted. One fixture app passes lint → `_validate` → `validate_envelope` in Python and renders every record with no placeholder in Svelte from the same exported JSON. `/meta` is server-owned (composedAt only), so `AppHeader.note` / a bound `Section.caption` target a source field and any other `/meta/*` binding is rejected — codex caught the draft binding `/meta/reach`, which would have rendered blank forever) | #630 |
...
| `NOUS_RETRIEVAL_TELEMETRY_ENABLED` | `true` | F091 master switch for retrieval telemetry. Records what recall retrieved AND which gate dropped everything it didn't, across **all three** retrieval paths — `pipeline` (the `recall_deep` tool), `context` (`ContextEngine.build`, every turn; previously instrumented only by a bullet-count regex over rendered prose at `context_logger.py:165`), and `script` (`recall_deep()` called inside `run_python`, added 2026-08-25 — until then that function was `heart.search_facts` and never reached an instrumented call at all, so in-script recalls were absent from the dashboard because no retrieval existed to log, not because rows were dropped). The path set lives in `nous/observability/retrieval_logger.py::RETRIEVAL_PATHS` and is what `/dashboard/retrieval` validates its filter against; `test_retrieval_paths_are_registered` AST-scans every `.start(path=…)` literal in the tree and asserts the two agree **in both directions**, so a new path can neither be emitted without a filter nor advertised without a producer. Ships **ON**: header rows, per-leg summaries and graph-expansion edges are cheap, and a telemetry system that lands dark measures nothing. The collector is **write-only** — the pipeline writes in and nothing reads back out — so with the flag off there is no branch consuming trace state and results are byte-identical by construction (`PipelineResult` stays frozen; the `recall_deep` snapshot and `nous_eval` contract are untouched). Verified by `scripts/diag/f091_pipeline_probe.py` and `f091_context_probe.py`, which assert results/system-prompt identity traced vs untraced. |
...
| `NOUS_RETRIEVAL_TELEMETRY_MAX_CANDIDATES` | `600` | F091 hard per-row candidate cap; hitting it sets `truncated` and logs WARNING (never silent). **Raised 300 → 600 by C1**, which registers the chunk leg's discard set — the largest population on the path, and a **constant** one, since the chunk allotment is deliberately decoupled from the caller's `limit`. At the live config (`NOUS_EPISODE_CHUNK_RECALL_LIMIT=30` with **no** `NOUS_CHUNK_RRF_PENALTY_LIMIT` in either `.env`), `limit_expanded = 90` per leg → up to `2*90 − 30 = 150` discards. **It does not breach at the default call**: at `recall_deep`'s default `limit=10` the pre-C1 population is ~95 typical / ~135 worst (heart survivors ≤10; heart cut ≤30, i.e. 2 types × `limit*2` since `coherent_ranking_enabled` strips censor+procedure; 30 served chunks; brain ≤10; graph ≤15 one-hop or ≤40 spreading; keyed/exemplar 0 while dark) → ~245/~285 with C1, under 300. It breaches on three live paths: (1) `limit` is LLM-chosen over 1–50 and heart/brain scale with it while the chunk 180 does not — `limit=25` → ~300, `limit=30` → ~345, `limit=50` → ~445; (2) the C1 acceptance replay runs at `candidate_sample_rate=1.0`, so the instrument's own validation would truncate; (3) every land-dark leg intended to flip (keyed +8, keyed_r2 +8, exemplar +25, `heart_graph_all_types`). A breach costs *identity, not totals*: `finalize` force-creates served items past the cap and C1 drains discards last, so the discards are solely what truncates — but a gold chunk cut at the merge and past the cap is then recorded nowhere and still reads `never_retrieved`, the exact miscount C1 exists to fix. `Leg.n_dropped` is exact regardless of this value. |
...
| `NOUS_A2UI_ENABLED` | `true` | F092 master switch — gates the `/a2ui/*` routes, the SSE stream, and `push_surface` registration (503 / unregistered when false). Ships ON deviating from land-dark deliberately: purely additive surface on an already-unauthenticated-behind-oauth2-proxy REST layer, and a push surface that lands dark demonstrates nothing. Kill switch only. |
| `NOUS_A2UI_OUTBOX_REPLAY_WINDOW` | `500` | F092 — max outbox rows replayed on SSE reconnect before the server sends a `resync` control event and the client re-hydrates from snapshots. |
...
| `NOUS_A2UI_COMPOSE_ENABLED` | `true` | F092.1 — register the `compose_surface` tool (ephemeral micro-apps: read-only, navigable UI composed fresh per intent, lives until closed). Ships ON with the same rationale as `NOUS_A2UI_ENABLED` — every compose output passes schema + structural + grammar validation with ≤2 repair rounds and a markdown fallback before it can reach a client. Kill switch only. |
| `NOUS_A2UI_COMPOSE_MODEL` | `""` | F092.1 — model for the compose LLM call. Empty = falls back to `NOUS_BACKGROUND_MODEL`. |
| `NOUS_A2UI_COMPOSE_TIMEOUT_SECONDS` | `60` | F092.1 — per-round timeout on the compose LLM call. A round runs inside a synchronous `/a2ui/call` or tool dispatch bounded by `NOUS_TOOL_TIMEOUT`; keep `(repairs+1)×this` under that or the outer cancel swallows the real error. Timeout is terminal (markdown fallback), never retried as a repair round. |
| `NOUS_A2UI_COMPOSE_MAX_PER_HOUR` | `60` | F092.1 — hourly in-process cap on compose LLM rounds (initial compose + repairs + every `app.refine`). Breach surfaces as the caller's error (422 / tool error), never a silent fallback. Mirrors the F050 budget pattern. |
| `NOUS_A2UI_APP_STALE_AFTER_S` | `3600` | F092.1 — seconds after `composed_at` before a micro-app's freshness stamp degrades to amber, client-side. Rendering only; nothing server-side fires on it. The stamp is a data-model binding (`/meta/composedAt`) so `app.refresh` restamps it without touching components. |
| `NOUS_A2UI_AGENT_SCRIPT_SOURCE_ENABLED` | `false` | F095 — register the `agent_script` dashboard source: **the agent writes the code that produces an app's data.** Every other source is a fetcher someone added in code, so a domain with no fetcher could only be a model-supplied SNAPSHOT — and an app with no `data_sources` is refused a refresh outright (`compose.refresh_data`), which is exactly what made it no better than the emailed static HTML this feature replaces. Here the script is stored in `app_spec.data_sources[].params.code` and `refresh_data` re-resolves it, so the app is genuinely **live for any domain with no new code and no new env var** — including one that reaches an external API, since the script has full builtins. Execution reuses the `run_python` sandbox **verbatim** (same run slot, wall-clock deadline tracer, settrace shims) via a second return shape (`run_script_structured` returns the object the script left in `result`, instead of MCP text) — a second executor would have meant a second, weaker set of guards. **Memory writes are disabled on this path regardless of this flag**: a stored script re-runs unattended on every refresh, so `learn_fact` would repeat with nobody in the loop; reads (`recall_deep`, `recall_recent`, `list_tasks`) stay in scope because reads are what a dashboard needs. Scripts can enter **only** through the agent's deliberate `compose_surface` tool call — `data_sources` is a parameter to `compose()` and is stored from that parameter, never from the compose LLM's parsed JSON, so the inner layout model and its repair loop cannot inject or rewrite executable code. **Default OFF — land-dark.** The other A2UI switches ship ON as kill switches because they gate a rendering surface; this one opens a new **exposure window** — code stored now and re-executed later on refresh, with nobody in the loop. By the same reasoning as `NOUS_DAG_CALLBACK_EXECUTION_ENABLED`, a change of that kind is flipped deliberately by an operator rather than acquired on deploy, and no measurement of real usage exists yet to size the impact. That the agent already holds unrestricted `run_python` and `bash` is why this is SAFE to flip — not a reason to flip it for you. **Separate switch** from `NOUS_PROGRAMMATIC_TOOLS_ENABLED` so an operator can stop stored scripts from re-running without losing interactive `run_python`: different exposure windows. Code is capped at 16k chars and a RESULT at 256k (`_bound`'s list trim pops one entry and re-serializes the rest per iteration — quadratic, on the main event loop, outside the script's deadline). The result is JSON-normalized **inside the worker**, under the deadline tracer, with `default=str` and `allow_nan=False`: `default=str` can invoke an agent-authored `__str__`, so running it on the loop after the slot is gone would let a blocking method stall every request, and NaN/Infinity are emitted by json as non-standard tokens that Postgres JSONB refuses at COMMIT time. The source then reads the already-encoded size and never re-touches the raw object. A source declares `params.shape` (`records`|`series`, default `records`) so a failure returns a value the app's EXISTING bindings still accept — refresh does NOT re-run binding validation. `series` fails as an empty series carrying the reason (the chart renders its §3.1 empty state with the cause, not "not a series (object)"); `records` fails as a one-row `[{_error}]` list so a table's list paths stay valid and the row shows what broke. A success whose value contradicts the declared shape is rejected the same way, so a chart can never bind to records. One broken source never fails the whole refresh. **Ops note:** dashboard scripts are capped at **2** of the 4 `run_python` slots, so a stalled one can never starve the agent's own interactive scripts. That cap exists because the deadline pushed into the worker stops **Python-level** code only — a script blocked inside a C call (`urlopen`, `time.sleep`) cannot be interrupted at all and holds its slot until it returns. That is a documented property of `run_python`, not something this source introduced; what this source changes is how REACHABLE it is, since fetching an external API is the advertised use and refreshes recur unattended. The shared source-resolution budget is pushed DOWN into the worker (cancelling only the await would leave the thread holding its slot until its own 90 s deadline, starving later calls). That budget is derived per PATH: **compose** reserves **every** round the composer may run (`MAX_REPAIRS + 1`, not one — an invalid first response costs up to two more), so at the default 120 s tool timeout there is no room and compose-time scripts get the 5 s floor; **refresh** reserves nothing, because `app.refresh` re-runs fetchers only and never calls the LLM, so it gets the full 45 s cap. Both clamp to [5 s, 45 s]. A script whose data is gathered at refresh (the normal case for a live dashboard) is therefore unaffected by the compose squeeze; if you want long compose-time scripts too, raise `NOUS_TOOL_TIMEOUT` — prod already runs 2000. The worker's own deadline is set SHORTER than the registry's wait so a routine script timeout returns the shape-preserving failure instead of cancelling the whole resolve. |
| `NOUS_A2UI_AGENT_ACTIONS_ENABLED` | `false` | F092.2 — allow `compose_surface` to declare `agent_actions` ({id,label,instruction} ≤4, tool-layer validated): server-stamped AppFooter buttons whose tap posts `app.act` through the full action gate and spawns a **background agent turn** (subtask, frame `task`) that follows the stored instruction and updates the app in place via `compose_surface(dedup_key)` — dedup replacement preserves surface_id and replaces the data model wholesale, clearing the `/meta/pendingAction` stamp for free. Actions enter ONLY via the agent's own tool call (F095 trust split — the inner compose LLM cannot author them, and `allowed_actions` is server-constructed so it cannot widen the allowlist either; `push_built` fail-closes to `{app.close, app.act}` and app.act only when app_spec declares actions). Server-side double-tap guard: a fresh pendingAction rejects the next tap, because each tap is an LLM turn against `NOUS_SUBTASK_MAX_CONCURRENT=3`. Failure honesty is two-layer: an in-process watcher clears the stamp + writes `/meta/actionError` on subtask failure/timeout/completed-without-recomposing (only when the stamp is still its own), and — because the watcher dies with the process (the F087 restart hole) — the client independently renders a stale note and re-enables the buttons past 5 min off the stamp's timestamp. **Default OFF — land-dark**: a UI tap becomes an LLM turn holding tools, executing an instruction stored earlier with nobody in the loop (same exposure-window reasoning as `NOUS_A2UI_AGENT_SCRIPT_SOURCE_ENABLED` / `NOUS_DAG_CALLBACK_EXECUTION_ENABLED`). The compose tool schema advertises the param only when the flag is on. |
| `NOUS_A2UI_AGENT_ACTION_TIMEOUT_SECONDS` | `300` | F092.2 — watcher deadline for the app.act subtask AND the double-tap freshness window (a second tap inside it is refused). |
| `NOUS_A2UI_MAX_LIVE_APPS` | `8` | F092.1 §6.3 — concurrency cap, not a TTL (micro-apps have `expires_at NULL`). On overflow the least-recently-**touched** app (`updated_at` — refresh/refine bump it; the server never learns about views) is resolved with a `deleteSurface` and a log line. "Until you close it" is honored for everything plausibly still in use; it does not mean unbounded accumulation. Dedup update-in-place never evicts. |
| `NOUS_A2UI_HEALTH_DB_PATH` | `""` | **F094 — the only new setting in F093+F094 (#616), and it is optional.** Path to an external health SQLite db backing the `health_series` chart source. Empty ⇒ that ONE source is not registered and everything else (themes, chart primitives, `to_series`, `decision_outcomes_series`, `dag_throughput_series`) works unchanged — the registry registers each source only when its backing component is wired, so a deployment without the health integration simply lacks it rather than erroring. The db and its schema are owned by that integration, not this repo, so the reader is defensive against a contract table `health_metrics(metric TEXT, ts TEXT ISO, value REAL)`: a missing db, missing table or drifted schema returns an **explicit empty series with a reason**, never a blank box or a confident zero. Reads are bounded in SQL (`ORDER BY ts DESC LIMIT`, default 2000, ceiling 5000) and run via `asyncio.to_thread` — an unbounded `fetchall()` on a multi-year metric would block the event loop for every other request during compose/refresh. |
...
| `NOUS_PROGRAMMATIC_TOOLS_TIMEOUT` | `90` | Timeout in seconds for run_python code execution. Enforced in-thread by a `sys.settrace` deadline hook, so Python-level runaway code (`while True: pass`) is actually interrupted; a blocking C-level call still cannot be interrupted and holds its slot until it returns. **Raised 10 → 90 (2026-08-25)** when in-script `recall_deep` stopped being `heart.search_facts` and became the real `run_recall_pipeline`. Measured on prod `nous_system.retrieval_log` (n=104): a pipeline retrieval runs **p50 5.3s / p95 14.7s / max 54.7s**, so the old 10s budget could not fit even ONE call reliably — and batching several is the tool's stated purpose. 90s fits ~6 calls at p95 and must stay under `NOUS_TOOL_TIMEOUT` (default 120s), which wraps every tool in `_dispatch_with_keepalive`; if the inner deadline ever met the outer one, the outer cancel would fire first and swallow both the per-script error message and the F091 partial-trace commit. The guard uses `programmatic_tools_timeout + 2s` — `run_python` awaits its worker on `timeout + PROGRAMMATIC_TOOLS_TIMEOUT_GRACE_SECONDS`, so checking the nominal value alone would accept `119` against `120` while the inner wait ran to 121s and lost the race anyway. Enforced by a cross-field validator, which **raises only when you set BOTH** and they conflict. If you set only `NOUS_TOOL_TIMEOUT` too low, the default is **clamped to `tool_timeout - grace - 1` with a WARNING** — raising there would turn an existing valid deployment (e.g. `NOUS_TOOL_TIMEOUT=60`, set long before this field had a 90s default) into a boot failure on upgrade, which is worse than the inversion the guard prevents. Prod runs `NOUS_TOOL_TIMEOUT=2000`, so it is unaffected either way. **Operator note:** a run now holds one of `NOUS_PROGRAMMATIC_TOOLS_MAX_CONCURRENT` (4) slots for up to 90s rather than 10s — a 9× longer worst-case hold on a fixed-size pool. Lower this if scripts start queueing. |
...
| `NOUS_SMART_COMPRESS_EXEMPT_TOOLS` | `["recall_deep","recall_recent"]` | Tools whose output SmartCompress passes through untouched. SmartCompress runs at **ingestion time** (`runner.py`, right after dispatch — not on history aging), so what it cuts the model never sees. It ranks by **original position** (30% head + 15% tail) and by `_score_line`, a heuristic scoring a line on whether it holds a digit, URL, file path or error word — it never reads the retrieval score. Applied to memory recall, which is the curated output of RRF + relevance floors + MMR + CE rerank + graph scoring, that is a second and far cruder ranker overriding a careful one. **Measured** on 25 prod-shaped blocks from real prod content: `is_crushable` fires **25/25** (uniqueness ratio on all 25, error regex on 22 — so neutering the regex alone would change nothing), blocks cut **225 → 75 lines**, and graph/spreading rows survive at **43.1%** against **92.0%** for top-10 heart rows — a 7× asymmetry against exactly the rows ranking work aims to promote, and the reason a scoring fix alone cannot reach the model. **Cost:** +5.4k tokens/call measured end-to-end at `recall_deep`'s default `limit=10` (~+8.3k at `limit=50`); ~72% of a block is chunk content, whose allotment is flat at `NOUS_EPISODE_CHUNK_RECALL_LIMIT`. Size control for these tools belongs in the pipeline where scores are known, not in a downstream text cutter. `[]` restores compression for every tool (kill switch). |
...
| `NOUS_SPREADING_ACTIVATION_DENSITY_THRESHOLD` | `3.0` | Density threshold for auto-enable. Prod density measured **3.776** on 2026-08-23, so `auto` fires on every recall with a decision or fact hit — spreading is LIVE, and it **replaces** the 1-hop leg rather than augmenting it (`retrieval_pipeline.py`, Stage 4 is either/or). |
| `NOUS_SPREADING_ACTIVATION_DECAY` | `0.5` | Per-hop activation multiplier. **Bounded `(0.0, 1.0]`** — a value above 1.0 amplifies activation per hop and breaks the `activation <= seed_score <= 1` invariant the CTE's MAX aggregation relies on to keep spreading rows on the candidate score scale. Was an unbounded `float`. |
| `NOUS_SPREADING_ACTIVATION_MAX_DEPTH` | `2` | Max traversal hops. **Bounded `[1, 3]`** — this is the exponent of an exponential traversal (prod p99 node degree 55, max 248), so each extra hop is an order-of-magnitude cost step. Was an unbounded `int`. |
| `NOUS_SPREADING_SCORE_DEPTH1_PARITY` | `false` | **F022 C-S.** When true a spreading row scores `activation / spreading_activation_decay` instead of `activation * graph_recall_decay`, correcting an **off-by-one in the decay exponent**. The CTE decays every hop *including the first* (`decay^depth`), while the 1-hop leg spreading **replaces** (Stage 4 is either/or) scores the identical `(seed, edge, neighbour)` triple as `seed * w` undecayed — the extra `graph_recall_decay` made the gap **2.857×**. The fix gives `decay^(depth-1)`: depth-1 at exact parity, each *additional* hop still discounted. Bound holds (depth-1 = `seed * w`, `w` clamped to 1.0, so ≤ `seed` ≤ 1). **Measured** over 64 prod retrievals / 848 rendered spreading rows: today **and** the weaker "just drop `graph_recall_decay`" variant (×1.43) both put **0** spreading rows in the top 10 — ×1.43 lifts the peak only to 0.479 against a 0.72–0.83 cutline, so it is a **measured no-op**. Parity puts **2** there (peak 0.958) at 0.03 evictions/call. Modest — depth-1 is 92% inferred edges averaging weight 0.41 — but a **prerequisite**: while spreading rows cannot rank, upstream C-R/C-W work cannot show up. **PRECONDITION** (codex P2 on #604): parity is defined relative to a *scoring policy*, so the branch is gated on the policy in force — `graph_neighbor_seed_score_enabled=true` **and** `graph_inferred_edge_penalty >= 1.0`, which is prod. At seed 0.8 / w 0.9 the 1-hop leg returns 0.7200 there, but 0.6300 with seed-score off and 0.5040 under an F065 penalty of 0.7, while the parity branch always returns 0.7200 — ungated, this flag would mean parity, a mild promotion, or a large one depending on two unrelated settings, and the C-S arm could not attribute its own result. Where the policy does not match it goes **inert** (today's scoring) rather than silently promoting. The penalty cannot be mirrored onto a spreading row instead: an activation composes several edges of mixed provenance, so there is no single `extraction_method` to price. **Default OFF** — a ranking change with no relevance oracle yet to judge whether the 2 promoted rows beat what they evict. **Membership-invariant by construction** (floor and cap act on `activation` *before* scoring), so recall@served cannot move; if it does, the harness is wrong. |
...
| `NOUS_EXECUTION_LEDGER_PERSIST_ENABLED` | `true` | Harness Phase 1b: persist every SIDE-EFFECTING tool call (`classify_side_effect != none`) to `nous_system.execution_ledger` (migration 074). `bash` is classified over the WHOLE command (`nous/cognitive/bash_side_effect.py`): redirections, pipes and chains, command substitution, the writing modes of read-only tools (`find -delete`, `sed -i`, `sort -o`, `git branch -D` ...), tools whose purpose is another host (`ssh`, `scp`, `rsync`, `nc`, `kubectl`, `git fetch` ...) as `external`, and wrappers (`sudo`, `timeout`, `xargs`, `bash -c`, `eval`) by what they run, with shell reserved words (`if`/`then`/`do`/`!`/`{` …) skipped since harness 2c; a path-qualified name (`/usr/bin/curl`) is classified as that program but never reads below `write` (`./cat` may be anything); a quoted or escaped operator character is a word, an assignment that can change what runs (`PATH=.`, `LD_PRELOAD=`, `GIT_*`) is `write` (only locale/display variables are exempt), and every recursion path (`eval`, `bash -c`, `find -exec`) shares one work budget whose exhaustion is `write`, and a command over 64 KB is `write` without being lexed, since classification runs on the event loop — a false `write` costs a row, a false `none` loses the record, so every ambiguity is `write`. It exists because the in-memory F026 ledger is dropped at session end, eviction and restart and has no ids. The row is written `pending` BEFORE dispatch and closed `success`/`error` after; a call cancelled mid-flight (subtask timeout, client disconnect, stream tool timeout) closes `unknown`, because its side effect may or may not have happened; a call the harness refused (offered-set `enforce`, ActionGate `enforce`) is one `blocked` row. Rows carry the `ExecutionContext` (`context_kind`, `session_id`, `parent_session_id`, `subtask_id`, `dag_id`, `dag_node_id`). `key_args` keeps a value only when its SHAPE proves it is not free text — a UUID, a lowercase enum word, validated email/chat recipients, a URL cut to `scheme://host` — or when it is the call's target path; every other value (bash commands, subjects, names, bodies, code) is stored as sha256+length and unlisted tools store argument NAMES only (while every one is a lowercase word; else a count + hash), a tool name nothing registered leaves no row at all, because pattern redaction cannot see a bare `sk-...` key. Tool output is never stored either — handlers echo their arguments and code prints anything — only its shape (length, bash exit code, a timeout note); a refusal is stored as a code (`offered_set` / `action_gate`), never the ActionGate model's prose, which is written from a prompt carrying the call's arguments; a value that fails to parse is hashed, and the bash classifier is total (an unanticipated input is `write`), so no input can make a call skip its row. Retention never deletes a `pending` row. Writes fail OPEN in Phase 1b (the call proceeds, WARNING logged); `idempotency_key`/`external_ref` are reserved for Phase 2b. Assumes ONE Nous process per (database, agent_id): the startup sweep marks every leftover `pending` row `unknown`. Kill switch only. |
...
| `NOUS_CLAIM_VERIFICATION_MODE` | `enforce` | Claim verification mode (shadow/warn/enforce). Harness Phase 2c. **What is a claim:** only the agent's own completed action — "I saved…", "I've pushed the fix", "I saved X and sent Y", plus actor-less completions with a completion shape ("was saved to <path>", a line-initial "Email sent to …"). Not a claim: narrating another agent's work ("the node ran and sent the email", "As I wrote earlier, …", "was saved to x **by** the scheduled task"; the `and <verb>` rule is clause-scoped), a question (judged by the next clause end, so "I pushed the fix — want me to deploy it?" still counts), a condition earlier in the clause ("check whether it was saved to…"), quoted text (fenced code, `>` quote lines, "…" spans), a lowercase "i", push/commit whose object is not headed by a version-control noun ("I pushed an approval card" is `push_surface` wording; "I pushed back", "committed to a code freeze"), a deploy with no build/release/service/destination, or an effect that landed in memory, a fact, the companion app, a micro-app or the reply itself ("the report below"). The object of a claim is inside one clause and stops at a relative or infinitive clause ("I created a schedule **that** generates the report" is not a file claim); a target is only where the object was saved. **What grounds it:** any tool capable of the effect that succeeded with arguments showing it — `write_file`, a bash write, or a script that writes a file; `send_email`, `send_file`, a mail client (`mail "$TO"` counts)/`msmtp`/`swaks`, SMTP or Telegram `curl`, or a script using `smtplib`; `git push` (not `-n`/`--dry-run`/`--help`) or `docker push`; `git commit` (`-n` is `--no-verify`); for a deploy an allowlist of changing subcommands (`compose up`, `kubectl apply`, `helm upgrade`, `systemctl restart`, `aws s3 sync`, `fly deploy`, `make deploy`, `vercel --prod`) — `docker ps`, `kubectl get`, `tail deploy.log` look at a deployment; Nous's own producers (`compose_surface`, `push_surface`, `learn_fact`, `ingest_document`) ground an untargeted file/message claim — and a named path or recipient must match. Bash is read with the ledger classifier's own lexer (`bash_side_effect.command_invocations`), including command strings (`bash -c`, `eval`, `env -S`, `flock -c`, `watch`, `ssh host CMD`, depth 3), `python -c CODE` and a heredoc-fed `python3 -` as code, and a heredoc body as one argument of its command (never as commands — `cat > runbook.md <<EOF git push EOF` is not a push), so `git push … \| tail -n 5`, `if git push; then …` and `ssh host 'git push'` push while `echo "git push"`, `bash -c 'ls'` and `ssh prod uptime` do not. **Unreadable is not no-signal:** a command the reader cannot parse (a quote cut in half, a ledger command past 64 invocations, a `$(…)` program) cannot disprove a claim and counts as plausible, and so does an interpreter with a script, a script or task runner whose text hints at the effect (`python3 scripts/daily_brief.py` may have mailed; `python3 --version`, `uv sync`, `npm test`, `pytest …` cannot have pushed anything) — under `enforce` a false violation misleads the agent, so only a command that was READ and did something else is disproof. `exact` needs a command that **certainly ran and succeeded** (`bash_side_effect.command_runs`): only the last AND-OR list's status is known from the exit code, so a command after `||`, or before a `;`, may have been skipped or masked and is plausible (`false && git push || true` is plausible, never exact — which branch ran is unknowable, and a false violation on `cd repo && git push || echo failed` would be the worse failure under `enforce`); a pipeline's status is its last stage's, so a piped push (`git push \| tail`) is plausible, and so are a negated (`! git push`), backgrounded (`git push &`) or substituted (`echo $(git push)`) one, while a constant untaken branch (`if false; then git push; fi`, `false && git push`, an unmatched `case` arm, an arm after one taken) or an uncalled shell function body is no evidence at all — **the reader resolves control flow only where a constant decides it and reports the rest as uncertain (plausible); it is not a shell or a Python interpreter, and a construction built to fool it is an accepted limit**; Python evidence is read from the syntax tree, over what the script *runs* (an uncalled `def`, an `if False:`/`while False:` branch, what follows a `return`/`raise`/`exit` on a path certainly taken, or an `except` handler is not evidence — unless the `try` body certainly raises a class the handler certainly catches; an `import smtplib` sends nothing, a shell API is only a qualified name or an alias an import proves, `False and x` never evaluates x, a library call named for the effect with a matching recipient keyword is plausible; a called shell function keeps its own branches; a copy into a directory lands under the source's name; `sendmail -t` reads its recipients from a heredoc and is plausible from a file) — the write's destination, the recipients written out (`sendmail(from, TO, …)`, `msg['To']`), a `subprocess` call read as argv, a call resolved to its own definition — never a sender, a comment or a printed string; code the ledger had to cut is unreadable (plausible), never regex-scanned; a program named by a path is the trusted tool only from a system directory (`./git` is not git); a ledger copy of this turn's call is never pooled beside the untruncated turn evidence; a non-zero exit disproves only an effect of the last command, and a failed opaque last command is no evidence at all; a named path or recipient compares whole (`write_file` to `/var/archive/report.md` is not "saved to /tmp/report.md"; `malice@x.io` is not `alice@x.io`; `send_file` never grounds an addressed claim); a bash save must **write to that destination** — an output redirect, a `cp`/`mv`/`tee`/`rsync` target, `-o`/`--output`, `sed -i`, `tar czf` — so `rm x` and `touch x.bak` never ground "saved to x", while a script naming the path in an unknown option stays plausible; a mail's recipient is read from its recipient **arguments** (curl: only `--mail-rcpt`), never its subject, body, data or attachments; a read command naming the path is not a save to it; an unquoted `#` comment is cut before reading (`echo ok # ; git push` runs no git). The ledger reads a command's invocations at record time from the whole command; this turn's arguments are checked untruncated; everything is linear, since it runs on the event loop. `enforce` injects a correction into the next turn (streaming cannot unsend). The `f026_claim_verification` event records every claim with its evidence level (`exact`/`plausible`/`none`), `claim_count` and the `turn`, so precision is measurable. |
...
| `NOUS_TOOL_OFFERED_SET_ENFORCEMENT_MODE` | `warn` | Harness Phase 1a: what happens when the model calls a tool it was NOT offered in that iteration (both `_tool_loop` and `stream_chat`, via one `AgentRunner._authorize_tool_call` choke point that Phase 2a extends with the capability policy). Every per-context restriction - subtask exclusions (`spawn_task`/`schedule_task`/`spawn_sync`), `tool_filter` for dynamic checks and callbacks, F078 refuse stripping - edits only the schema list; the dispatcher resolves any registered name, so such a call runs. `off` = no check; `warn` = run it, log WARNING, persist a `harness_unoffered_tool_call` event (`tool_name`, `context_kind`, `mode`, `offered_count`) to `nous_system.events` (gated by `NOUS_F026_PERSISTENCE_ENABLED`); `enforce` = refuse with a paired tool error and a `blocked` session-ledger row, never dispatched. Ships `warn` because prod dynamic-check callbacks with `on_complete_tools=['bash']` are prompted to notify via Telegram and currently succeed only through this hole - flip to `enforce` after reading the events. `extra_tools` (e.g. `submit_final_report`) count as offered. Every `run_turn` caller now passes an `ExecutionContext` (`nous/api/execution_context.py`: `interactive`, `mcp`, `subtask`, `dag_node`, `scheduled`, `agent_action`, `heartbeat_triage`, `heartbeat_check`, `heartbeat_callback`, `dag_summary`, or the generic `background`), threaded `run_turn -> _tool_loop -> ToolDispatcher.dispatch`; an AST test fails if a new caller omits it. |
| `NOUS_TOOL_CONTEXT_POLICY_MODE` | `warn` | Harness Phase 2a: per-context tool policy (`nous/api/tool_policy.py`), consulted at the same choke point as the offered-set check (`AgentRunner._authorize_tool_call`, both loops). Every tool's class (`none`/`write`/`external`/`irreversible` + `spawns`) is declared once in `nous/api/tool_classes.py`; the ledger's `READ_TOOLS`/`WRITE_TOOLS`/`EXTERNAL_TOOLS`/`IRREVERSIBLE_TOOLS` and the F078 refuse denylist derive from it (the denylist now also strips the 8 tools that had no class — `ingest_document`, `resolve_decision(s)`, `spawn_sync`, `dag_create`, `dag_manage`, `push_surface`, `compose_surface`), a drift test fails on any `dispatcher.register()` without an entry, and `run_python` code or the code a `python` command runs right there (`-c CODE`, `-cCODE`, a `-Ic` bundle, a heredoc or here-string on stdin) that reaches the network — a network module imported or used (`smtplib`, `requests`, `httpx`, `urllib[3]`, `http.client`, `socket`, `imaplib`, `websockets`, …), a connection helper, a URL literal, or `curl`/`wget`/`ssh` run through a shell; the English word in a string does not count, a URL in a comment does (erring toward external by design) — classes as `external` instead of a local write. A script FILE (`python x.py`, `uv run …`) or code piped in cannot be read and stays at the `write` floor, so the local-only rows below are not proof against a script on disk. Rows: `interactive`/`mcp` may do anything; `subtask`/`dag_node`/`scheduled`/`agent_action` may send but not spawn; `dag_summary` may spawn only `spawn_task` (prod delivers DAG mail through it); `heartbeat_triage` and generic `background` are local-only (`none`/`write` — `send_email`, `bash curl` and a networking `run_python`/`python -c` alike); a `heartbeat_check`/`heartbeat_callback` with a declared tool list (`ExecutionContext.declared_tools`; an empty list is the DB default and means undeclared) may use only those tools, declaring `heartbeat_check_create` authorizes a check pipeline, and a callback may not resurrect its own check (`ExecutionContext.check_name`, compared normalized): neither `heartbeat_check_manage(action=enable)` nor `heartbeat_check_create` under the same name. Violation codes: `unclassified`, `undeclared`, `level:<level>`, `spawn`, `reenable`. `off` = no check; `warn` = run it, log WARNING, persist `harness_context_policy_violation` (`tool_name`, `context_kind`, `violation`, `mode`); `enforce` = refuse with a paired tool error and a `blocked` ledger row (code `context_policy`). The policy runs for every call the offered-set rule did not refuse, so under `warn` a deviation is recorded by both — but an offered-set refusal under `enforce` is never evaluated by the policy: flip the offered-set rule to `enforce` only after reading the policy's warn data, or the refused calls vanish from it. Ships `warn`: Phase 1a/1b/2c are not deployed yet, so no prod data exists — flip after reading the events. |
| `NOUS_TOOL_ARG_SALVAGE_ENABLED` | `true` | Dispatch-level repair for model-emitted tool input where a required arg leaked as Claude-internal XML syntax inside another string arg (observed 2026-07-13: `record_decision` description ending `</description>\n<parameter name="confidence">0.55`). When a schema-required key is missing, ToolDispatcher extracts a trailing `<parameter name="KEY">value` run from string args, type-coerces per schema, strips the leaked tail, and logs a warning. A repaired call still tells the model what it got wrong — the result is prefixed `[input repaired]` naming the recovered key, because a silent success teaches the model nothing and it emits the same broken shape next turn. **Two validations run independently of this flag**, both returning an actionable tool error instead of an opaque one: (1) missing required args name the missing + provided keys rather than raising a raw `TypeError` that leaks `create_nous_tools.<locals>` internals; (2) **structural type mismatches** name the field, the declared type and what was actually sent (observed 2026-08-22: `record_decision` with `tags='fannie-mae, cpm, condo, …'` reached the model as a raw pydantic `ValidationError` + docs URL). The type check is deliberately narrow — only container-vs-scalar confusion, since pydantic's lax mode legitimately coerces `"0.9"` → `0.9`, so rejecting scalar-for-scalar would fail calls that succeed today. Union types, absent `type`, and nulls are skipped. Wrong types are reported, never coerced: the model must learn the shape. Containers are never salvaged at all — knowing a leaked array is usable would require validating `items` + nested `required` + nested enums, i.e. a real schema validator, so salvage declines and the model is told. **Related invariant (not flag-gated):** every failure path in `tools.py` returns via the `_tool_error()` helper, which sets `is_error: True` — and, since harness Phase 1b, so does every failure in `builtin_tools`, `email_tools`, `telegram_tools`, `web_tools` and `identity/tools`, whose one success/failure helper had made a refused write or a rejected email read as `success` in the durable execution ledger (`test_handler_error_flags.py` forbids returning the success helper from any `except` branch there; bash's non-zero exit stays unflagged). Error returns previously omitted it — 33 single-line, 5 multi-line, plus `ingest_document`'s `_block` helper — so `dispatch` read `bool(result.get("is_error"))` as False and reported genuine failures to the model as successes: a decision that was never written came back as "recorded". Empty-result messages ("No matching decisions.", "No active DAGs.") deliberately stay unflagged — a search that found nothing succeeded, and flagging it would teach the model an empty corpus is a broken tool. Locked by an **AST-based** test, not a line regex: a line-based guard reports clean over a multi-line offender, which is worse than no guard because it stops anyone looking. Set `false` to disable salvage (all validation stays). |

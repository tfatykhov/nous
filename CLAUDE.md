...
| `NOUS_CLAIM_VERIFICATION_MODE` | `enforce` | Claim verification mode (shadow/warn/enforce). Harness Phase 2c: only **first-person** completion claims are checked ("I saved…", "I've pushed", "I saved X and sent Y", plus the actor-less "saved to <path>" / "email sent to …"); narrating another agent's work ("the node ran and sent the email", "I checked: the DAG … and sent …") is not a claim, and "I" is case-sensitive. A claim never spans a sentence. It is grounded when **any** tool capable of the effect succeeded with arguments that show it — `write_file`, a bash write, or a script that writes a file; `send_email`, `send_file`, a mail client or SMTP `curl`, or a script using `smtplib`; bash running `git push` (not `-n`/`--dry-run`) classified external; `git commit` (where `-n` is `--no-verify`); a deploy/transfer tool (`docker`, `kubectl`, `rsync` …) for a deploy — and a named path or recipient must match. Git and mail are read with the bash classifier's own lexer (`bash_side_effect.command_invocations`), so `git push … \| tail -n 5` is a push and `echo "git push"` is not. A bash call with a non-zero exit is not evidence. This turn's arguments are checked untruncated (the ledger keeps a bounded head+tail); every signal pattern is linear, since it runs on the event loop. `enforce` injects a correction into the next turn (streaming cannot unsend). The `f026_claim_verification` event records every claim with its evidence level (`exact`/`plausible`/`none`), `claim_count` and the `turn`, so precision is measurable. |

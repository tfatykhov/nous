...
def _later_turn_level(code):
    ledger = ExecutionLedger(session_id="s")
    ledger.set_turn(1)
    ledger.record("run_python", {"code": code}, "ok", "success")
    ledger.set_turn(2)
    return ClaimVerifier().verify("It was saved to /tmp/report.md.", [], ledger, turn_evidence=[]).claims[0].evidence


def test_truncated_ledger_code_is_unreadable_never_regex_scanned():
    # the cut lands inside a string literal: still parses, and a string is not a write
    assert _later_turn_level("s = '''open('/tmp/report.md', 'w')" + "x" * 40_000 + "'''\nprint(len(s))") == "none"
    # the cut breaks the code: unreadable, so plausible -- never a regex hit on the head
    broken = "open('/tmp/report.md.bak', 'w')\ndata = [" + "1, " * 20_000 + "]"
    assert _later_turn_level(broken) == "plausible"

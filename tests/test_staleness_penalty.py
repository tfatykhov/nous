...
class TestStalenessPenalty:
    ...
    def test_rule_category_exempt(self):
        engine = self._make_engine()
        item = FakeItem(score=0.8, created_at=datetime.now(timezone.utc) - timedelta(days=60), category="rule")
        result = engine._apply_staleness_penalty([item])
        assert result[0].score == 0.8
    ...
    def test_30_percent_floor(self):
        engine = self._make_engine()
        item = FakeItem(score=0.8, created_at=datetime.now(timezone.utc) - timedelta(days=365))
        result = engine._apply_staleness_penalty([item])

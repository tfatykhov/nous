...
**v2.2 (implemented; after the verify-by-execution reviewer):** `python -c` is read in every spelling (`-cCODE`, a `-Ic` bundle, a heredoc or here-string on stdin — the classifier now cuts heredoc bodies like the claim verifier's reader); the network detector matches a module IMPORTED or USED (`requests.get`, not the word in a string), the connection helpers, `urllib3`/`http`/`imaplib`/`poplib`/`websockets`, a URL literal, and `curl`/`wget`/`ssh` run through a shell; the reenable rule compares both names normalized and also refuses `heartbeat_check_create` under the callback's own check name (delete-and-recreate was a resurrection route); Task 3's callback tests live in `tests/test_heartbeat_dynamic.py` (where `_execute_callback` is tested), not `tests/test_heartbeat.py`; the streaming refuse strip has a test. Documented: an offered-set refusal under `enforce` is never evaluated by the policy, so the flip order matters.

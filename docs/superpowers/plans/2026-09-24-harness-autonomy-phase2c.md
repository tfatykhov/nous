...
**v2.20 (codex round 16 + the final independent verify-by-execution pass, 22 items):** shell: a called function body keeps its own branches (it is collected as lists like the script and replayed through the same frames), `f() ( … )` and `function f () { … }` are definitions, `;;&`/`;&` case terminators, the `esac` after a `;;` no longer swallows what follows (a false violation introduced in v2.18), a bare digit before a redirect is the file descriptor, open parentheses are counted not rescanned (40k unbalanced `(` in 37 ms, was 5 s). Python: an import is not an operation (`import smtplib` sends nothing; a deploy module counts only when called), a shell API is only a QUALIFIED name or an alias an import proves (`sp.run`, `from subprocess import run as sh`; a bare `run(...)` is the script's own), `False and x` / `x if False else y` / a comprehension over `[]` or filtered by `False` / an unconsumed generator never evaluate x, a write API's keyword destination (`filename=`, `fp=`, `path_or_buf=`), `Path(...).open('w')`, `io.open`/`codecs.open`, archive openers in a write mode, `os.replace`/`os.rename`, `shutil.copytree`/`make_archive`, a copy INTO a directory (`dir/basename`), the shell invocations a script runs consulted for file and mail claims, a Telegram URL read from an f-string/concatenation/`%`/`.format` prefix, a library call named for the effect (`yag.send(to=…)`, `repo.git.push()`) plausible with its recipient keyword compared, each call reading the instance binding before it. Paths compare normalized (`/tmp//x`, `./x` both sides); `cp x dir/` lands at `dir/x` and a tree copy is plausible for anything under it; `sendmail -t` reads To/Cc/Bcc from a heredoc and is plausible from a file. `_python_facts` is cached per script (a 436-claim reply × 5 scripts took 30 s on the event loop; now once per script). Accepted limits noted in the PR: a constant `cd` before a relative destination, starting an app as a deploy, `false || x` at exit 0 reported plausible rather than exact.

**v2.19 (codex round 15):** a pipeline's constant status is its LAST stage's (`true | false && x` skips x); a Python body ends at any statement that certainly leaves it — a taken constant branch or a `with` that returns/raises, an `if` both of whose sides leave, a `while True` with no `break`, a `try` whose body returns or raises what no handler catches — and every walked body is cut the same way; an `except` handler runs when the `try` body certainly raises a class it certainly catches (bare, `BaseException`, the same name, or a builtin base — the builtin hierarchy is fixed), and no handler when the first that may catch it is not known to; a producer or `send_file` is judged by the target alone before the names-only fallback (the runner's `evidence_args` is `{}` for them).

**v2.18 (codex round 14):** `false && x` / `true || x` are evaluated (the short-circuited pipeline never ran); a `case` on a literal word runs only its first matching literal arm (`*` matches; a variable word or a glob pattern is unknown); a Python body stops at an unconditional `return`/`raise`/`exit`, a `try` that raises skips its `else`, and instance types come from live assignments only, last in document order. The reader's scope is now stated in the Global Constraints and in both modules' docstrings.
...
- **Scope of the evidence reader (settled after 14 codex rounds, v2.18; rounds 15–16 and the final independent pass fell inside it):** the reader is not a shell or a Python interpreter. It resolves control flow only where a *constant* decides it — shell `if`/`elif`/`while`/`until` on `true`/`false`/`:`, `case` on a literal word, `false && x` / `true || x`, an uncalled function body, `!`, `&`, `|`, `$(…)`, heredocs and quoting; Python `if`/`while` on a constant, `for` over an empty literal, an unconditional `return`/`raise`/`exit` (also inside a taken branch, a `with`, a `while True` without `break`), an `except` handler on a certain raise of a class it certainly catches, a `try` that raises, an uncalled `def`, literal constructor assignments — and reports everything else as *uncertain*, which the verifier reads as **plausible**: by construction never a false violation. Its threat model is an agent's honest-but-wrong claim under `enforce`, where a false violation injects a wrong correction — not an adversary, who has unbounded means (`bash -c "$(base64 -d …)"`). A review finding is **in scope** when it is a constant-control-flow gap or an equality-vs-mention gap; **out of scope** when it needs a runtime value, data flow, or deliberate obfuscation — those are accepted limits. Every accepted false grounding also verified on `main`, where any successful bash call grounded any push claim; none is a regression.

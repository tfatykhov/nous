...
- **Scope of the evidence reader (settled after 14 codex rounds, v2.18):** the reader is not a shell or a Python interpreter. It resolves control flow only where a *constant* decides it — shell `if`/`elif`/`while`/`until` on `true`/`false`/`:`, `case` on a literal word, `false && x` / `true || x`, an uncalled function body, `!`, `&`, `|`, `$(…)`, heredocs and quoting; Python `if`/`while` on a constant, `for` over an empty literal, an unconditional `return`/`raise`/`exit`, an `except` handler, a `try` that raises, an uncalled `def`, literal constructor assignments — and reports everything else as *uncertain*, which the verifier reads as **plausible**: by construction never a false violation. Its threat model is an agent's honest-but-wrong claim under `enforce`, where a false violation injects a wrong correction — not an adversary, who has unbounded means (`bash -c "$(base64 -d …)"`). A review finding is **in scope** when it is a constant-control-flow gap or an equality-vs-mention gap; **out of scope** when it needs a runtime value, data flow, or deliberate obfuscation — those are accepted limits. Every accepted false grounding also verified on `main`, where any successful bash call grounded any push claim; none is a regression.

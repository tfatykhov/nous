...
> 3. **Persistence is richer than "strip and forget."** Per product direction (2026-06-17): originals are **saved to `workspace_dir/attachments/`** (already sandboxed by `builtin_tools._validate_path`) with a **Heart fact memory-reference** so they're retrievable later; text/code files are also **chunk-ingested into `episode_chunks`** (reusing the `ingest_document` core). Base64 still never reaches the DB. **PDF bodies are now chunk-ingested too (F024.1):** a hybrid `pypdf` text extraction with a **Claude transcription fallback for scanned/image-only PDFs**, on by default via `NOUS_ATTACHMENTS_INGEST_PDFS`. PDFs are still also analyzed in-turn + persisted + fact-referenced.
>
...
- **Scanned-PDF transcription truncation (F024.1).** PDF body chunk-ingestion is hybrid: text-based PDFs are extracted in full via `pypdf`, but PDFs with no extractable text (scanned/image-only) fall back to a Claude transcription pass capped at `NOUS_ATTACHMENTS_PDF_MAX_TRANSCRIPTION_TOKENS` (default 8000). A long scanned PDF whose transcription exceeds that cap is **truncated** (a `max_tokens` stop logs a warning); multi-pass paging of the transcription fallback is deferred. Text PDFs via `pypdf` are complete.

...
| **Gestalt Trap** | "The whole is more than the sum of parts" is a pseudo-explanation. Specify the interactions | Facts are NEVER retrieved in isolation. `run_recall_pipeline` chains vector seeds (L1) → 1-hop graph neighbors (L2) → conditional spreading activation (L3); co-occurrence edges (PR #475) + co-mention edges (F076) + recency resolver (`context.py:961-1011`) keep facts connected. No bare-fact endpoint. | ✅ Now addressed — the retrieval pipeline prevents bare-fact isolation by construction; every recalled fact carries 1-hop neighbors and optional spreading-activation neighbors. |
...
| Minsky Concept | Description | Nous Implementation | Gap |
|---|---|---|---|
| **One Self or Many** | Self is a society of sub-agents with different concerns | Single `agent_identity` per agent (one versioned story, `models.py:53-66`). Frames carry `suppressed_frames` + `agencies_to_activate` (mode-switching) but are NOT modeled as partial selves with distinct values. | Still missing — frames are mode-selectors, not sub-agents. The "each frame = a partial self with own defaults/priorities" idea is unimplemented; the system does not populate or enforce per-frame identity. |
...
| **Anti-Dumbbell (11.9)** | Always seek a third alternative before accepting binary framings | Confidence scoring + multi-reason deliberation on decisions; epistemic routing gate (§2) gates fact-vs-base-knowledge (`context.py:236-247`). | ✅ Implemented functionally, though not as an explicit third-option requirement gate. Low-priority gap. |
...
| **Don't Notice Too Much** | Over-attending to detail prevents generalization | F023 admission control gates what enters memory; admission scoring filters noise (`nous/heart/admission.py`). | ✅ Directly validates F023. |
...
| Minsky Concept | Description | Nous Implementation | Gap |
|---|---|---|---|
| **Right Representation** | Hard problems become easy with the right description | F050: `nous/heart/query_expansion.py` `QueryExpander` runs Haiku to generate semantic variants at recall time, expanding a single query into `[query, variant1, variant2]`. | Now addressed — query reformulation is shipped and operational (flag-gated; defaults match F050 settings). |
| **Multiple Descriptions** | Same problem should be re-described in multiple ways to find the best approach | F050 multi-query expansion (`query_expansion.py:66-93`): variants preserve intent while changing phrasing (synonyms, jargon vs plain, noun vs verb); all variants run through the same recall pipeline. | Now addressed — reformulation happens at recall time. |
...
| **Memory Rearrangement (15.7)** | Memories merge, compress, relink over time | Sleep Phase 4 compresses old episodes → summaries; Phase 5 reflects/generalizes (LLM cluster merge); F031 consolidation resolves contradictions (SUPERSEDE/MERGE/KEEP_BOTH); recency resolver tags current vs superseded (`context.py:961-1011`); co-occurrence edges form new links at sleep; `graph_densifier` orphan backfill relinks. | Changed — merge/compress shipped; relink partially addressed via co-occurrence formation + orphan backfill. Still missing: DYNAMIC re-weighting during conversation (weights static at creation). |
...
| **Distributed Memory (20.9)** | Memory distributed across agents. Recall assembles fragments | Memory is distributed across fact/decision/episode/procedure/chunk types connected by graph edges; F067 adds `episode_chunks`; spreading activation activates fragments across the graph at retrieval time. | Now addressed (distribution + activation) — spreading activation connects/activates fragments across types. But recall still returns stored entities; algebraic fragment ASSEMBLY (combine partial facts into one narrative) is still missing. |
...
| Minsky Concept | Description | Nous Implementation | Gap |
|---|---|---|---|
| **Default Assumptions** | Frames should pre-load expected context. "Debug" should assume recent errors | Frames carry `default_category`, `default_stakes`, `questions_to_ask`, `agencies_to_activate` (`models.py:77-80`), returned in `FrameSelection` (`frames.py:186-195`) and consumed by the context engine (`context.py:140-150`). | Now addressed (partial) — frame defaults are loaded and surfaced; deeper "default context pre-load" beyond `questions_to_ask` is still shallow. |
...
| Minsky Concept | Description | Nous Implementation | Gap |
|---|---|---|---|
| **Censors from Failure (27.1)** | When thought → bad outcome, create censor to suppress that thought BEFORE it forms | Sleep Phase 1 (`_phase_review_decisions`, `sleep_handler.py:546-558`) reviews pending decisions but does NOT generate censors; censors can be manually created or linked via `learned_from_decision`, but auto-learning is not wired. | **Still missing** — no auto-censor-from-bad-outcome loop despite the review-phase infrastructure being present. (F012 procedure learning partially captures error→recovery pairs; see Ch. 30 Mental Bugs.) |
...
| **Mental Bugs (30.8)** | Systematic errors persist because you never test that part of your model | Auto-censors from outcomes are still not wired, but F012 procedure learning auto-creates procedures from observed error→recovery pairs (`procedure_learner.py:260-294`), capturing failed paths so they aren't repeated. | Now partially addressed — F012 error→recovery capture partially implements this; full coverage still needs auto-censor generation (Ch. 27). |
...
|---|---|---|
| Gestalt Trap (no bare-fact isolation) | Ch. 2 | Recall pipeline chains L1→L2→L3; co-occurrence/co-mention edges + recency resolver |
...
| Co-activation / edge FORMATION from experience | Ch. 11 | PR #475 `build_cooccurrence_edges` + F076 co-mention (formation only) |
...
| Sleep Consolidation | Ch. 15 | Phases 1-5 active (review/prune/compress/reflect); contradiction consolidation (F031) | Auto-censor generation from bad outcomes |
...
| **Auto-Learned Censors** | Ch. 27 | High | Sleep → detect bad outcomes → create censors (review phase exists; generation not wired) |

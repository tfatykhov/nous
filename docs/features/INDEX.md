...
| F082 | [PPR Recall Leg](F082-ppr-recall-leg.md) | ✅ Shipped (default OFF) | Query-personalized PageRank over `brain.graph_edges` as an N-th recall leg. N-leg weighted RRF (`_rrf_merge_n_weighted`), power-iteration PPR (`nous/brain/ppr_recall.py`), density gate, autobehavior exclusions reused. Default OFF (`ppr_weight=0.0`); promote by setting `NOUS_PPR_WEIGHT>0`. |

...
| F082 | [PPR Recall Leg](F082-ppr-recall-leg.md) | 📋 Proposed | Query-seeded Personalized PageRank as a new recall leg over `brain.graph_edges`, fused via N-leg RRF. Closes the one real HippoRAG-v2 gap (global query-personalized PageRank) that F022 spreading activation (dead by hop 2) and static `recall_hubs` do not cover. Reuses existing `seed_score` reset vector + `autobehavior_exclusion_sql` edge model; power-iteration in NumPy, no igraph in hot path. Default-off (`ppr_weight=0`), telemetry-first rollout mirroring F044. |

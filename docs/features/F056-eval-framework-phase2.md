...
- v5 (2026-04-28): clarified §"Per-handler eval lifecycle" step 9 during F056 PR #1 review. The original "Exit 0 on pass / 4 on regression" wording assumed in-handler gating, but the project shipped per-harness gating in `regression.py` (Phase 1 finish + PR #371). Handlers don't have access to baselines without duplicating regression.py's fetch logic — so handlers persist to `eval_runs` and exit 0; the subsequent `python -m nous_eval.regression --harness <name>` reads the row and exits 4 on regression. Same gating outcome, cleaner separation of concerns. CI wires both as two steps (`handler && regression`).
...
9. Exit 0 on a clean run. Regression gating is the SEPARATE `python -m nous_eval.regression --harness <name>` step, which reads the row written by step 8 and exits 4 on regression vs prior baseline. CI wires both as two steps: `handler-eval && regression-check`. (Original v1-v4 spec said "Exit 0 on pass / 4 on regression" assuming in-handler gating; v5 clarified that gating lives in regression.py per Phase 1 finish, not the handler — see §"Review history".)

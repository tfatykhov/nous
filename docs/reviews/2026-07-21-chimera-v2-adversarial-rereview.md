...
- **"Qwen3-Next / Qwen3.5 (Alibaba, deployed)" (Doc 1 §2.1, §8 table) — half-verified.** Qwen3-Next with Gated DeltaNet is confirmed deployed. "**Qwen3.5**" as a *shipped* production system using DeltaNet is **not** confirmed by a primary source — the strongest reference is analysis/commentary ([mlabonne, HF blog](https://huggingface.co/blog/mlabonne/qwen35)), not a release. Drop "Qwen3.5" or mark it prospective.

### VERIFIED-BUT-MISFRAMED (the important one)

- **Qwen3-Next does not validate "DeltaNet gives retrieval/citation exactness."** The production system is a **3:1 hybrid**: ~75% Gated DeltaNet layers *interleaved with a full softmax-attention layer every 4th layer*, and the full-attention layers are there **specifically to carry retrieval fidelity** that linear attention alone does not provide ([vLLM blog](https://vllm.ai/blog/2025-09-11-qwen3-next): "full attention layers provide global context and strong retrieval capability, while linear layers provide efficient O(1)-per-token inference"). The one production precedent the document leans on is itself evidence that **DeltaNet alone was not trusted for exact retrieval** — it was paired with softmax attention. Doc 1's central inference ("citation exactness is a design property of the key scheme") is *undercut*, not supported, by the precedent it cites.
...
**Premise 1 — "The substrate swap structurally dissolves P1 #1 (composite vs. citation)."** *Partially true, overclaimed.* Switching the *write rule* from blind Hebbian sum (Hopfield) to error-correcting delta rule genuinely removes the superposition-catastrophe framing of round 1: a delta-rule write at key `k` erases the old association at `k` before writing, so single-key readout is clean-by-construction in a way no β could give Hopfield. That half is real and is the strongest idea in v2. **But the composite/multi-entity half relocates the exact same lossy-composition problem into the VSA bundling layer** (§3 below). "Structural dissolution" is earned for single-entity keyed recall; it is *not* earned for composites, which is where P1 #1 actually lived ("compose an answer across facts" was the whole value proposition).
...
- **Drop the VSA composite layer; keep DeltaNet single-key recall + Postgres composition.** If the ledger is doing the exact citation anyway (P1-B), compose answers in fact-space directly from ledger rows and use DeltaNet only for single-entity associative recall where its delta rule is genuinely clean. This removes the one mechanism (VSA bundling) whose own literature contradicts the exactness claim. Strictly simpler; likely captures most of the benefit.
...
7. Fix the "Yang, Wang" authorship citation and drop unconfirmed "Qwen3.5"; institute a check-citations-against-source habit (two rounds, two misattributions).
